...
Fixtures live in ``testdata/f2_samework_hard_cases.json``. Cases 5, 7, 13,
...
def test_container_comparison_is_exact_not_fuzzy():
    from cde.refs.samework import container_same_work
...
def test_language_never_reads_the_record_language_field():
    ...
    import inspect
    import re as _re
    from cde.refs import samework as m
...
_CORP = json.load(open(
    os.path.join(os.path.dirname(__file__), "testdata",
                 "f2_corporate_author_cases.json"),
    encoding="utf-8"))
...
def test_title_sim_is_not_a_path_into_entry_4():
    ...
    from cde.refs.biblio_match import title_sim
    from cde.refs.samework import title_equivalent
...
def test_entry_4_requires_reason_corporate_not_merely_None():
    ...
    from cde.refs.samework import author_evidence_detail, entry_corporate
...
def test_entry_4_leaves_disagreeing_personal_rosters_flagged():
    ...
    from cde.refs.samework import entry_corporate
    fires, why = entry_corporate(
...
def test_entry_4_runs_last_so_it_cannot_move_the_earlier_fixtures():
    from cde.refs.samework import entry_for
    for case in _FIXTURES:
