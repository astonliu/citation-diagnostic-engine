...
from cde.diagnose.f5_seams import build_f5_seams, make_f5_evidence_builder
from cde.diagnose.supersession import F5Policy
from cde.runtime.production_launcher import LaunchRefused
...
def test_a_dirty_tree_refuses(tmp_path, monkeypatch):
    monkeypatch.setattr(pl, "_git", _git_stub(tmp_path, "a" * 40,
                                              status=" M cde/diagnose/pipeline.py"))
...
def test_runtime_bytes_differing_from_the_commit_refuse(tmp_path, monkeypatch):
    ...
    pkg = tmp_path / "cde"
    (pkg / "diagnose").mkdir(parents=True)
    (pkg / "diagnose" / "pipeline.py").write_text("edited after checkout",
                                         encoding="utf-8")
...
def test_f6_cocitation_router_is_governed():
    assert "claims/cocitation.py" in pl.GOVERNING_MODULES
...
def test_unresolvable_paths_are_not_reported_as_an_integrity_failure(
    ...
    pkg = tmp_path / "cde"
    (pkg / "diagnose").mkdir(parents=True)
    (pkg / "diagnose" / "pipeline.py").write_text("x", encoding="utf-8")
