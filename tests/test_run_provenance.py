...
from cde.diagnose.provenance import F3Policy
from cde.runtime.parser_versions import CLAIM_PARSER_VERSION, COVERAGE_PARSER_VERSION
...
def test_module_hashes_are_captured_before_execution(tmp_path, monkeypatch):
    ...
    assert "refs/preband_contract.py" in manifest["module_sha256"]
...
def test_f7_provenance_block_exists_when_f7_is_wired(tmp_path, monkeypatch):
    ...
    assert "diagnose/entity.py" in manifest["module_sha256"]
...
def test_f7_is_absent_when_unwired(tmp_path, monkeypatch):
    ...
    assert "diagnose/entity.py" not in manifest["module_sha256"]
...
def test_a_version_string_in_the_digest_slot_is_refused():
    ...
    from cde.diagnose.entity import _validate_relation_comparison
