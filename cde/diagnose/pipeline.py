...
from ..claims import band as jb
from ..claims import cocitation
from ..claims import marker_scope
from ..refs import preband_contract as pc
from ..refs.preband_contract import PrebandContractError
from .engine import (
    ClaimSupport,
...
from ..claims.band_prompts import CLAIM_EXTRACT_PROMPT_VERSION, COVERAGE_PROMPT_VERSION
from ..runtime.recording_adapter import paid_call_meter
from ..refs.ncbi_meta import DEFAULT_EMAIL, RetrievalUnavailable
...
from .supersession import F5Policy, F5_REPORTABLE, decide_f5
from .entity import (
...
from ..claims import fulltext as ftr
from ..claims.aggregate import no_usable_fulltext_dict
from ..claims.coverage_prompts import (
    COVERAGE_PROMPT_VERSION_V3,
    RESPONSE_PARSER_VERSION as RESPONSE_PARSER_VERSION_V3)
from ..runtime.parser_versions import CLAIM_PARSER_VERSION, COVERAGE_PARSER_VERSION
...
from . import outcome as tox
from ..claims import citation_selection as csel
...
def judge_pair_coverage(item: dict, *, extractor, coverage_judge, fetch_abstract,
    ...
    from ..claims.band_prompts import evidence_is_usable
    ...
    if f5_seams is not None and f5_evidence_builder is not None:
        try:
            ...
            if effective_f5_policy.mode == "deployment":
                from .evidence_store import source_packet_from_dict
                from .supersession import validate_f5_record
                packet_rows = getattr(
...
def _module_hashes(fulltext_path: bool, f5_seams, f7_seams) -> dict:
    ...
    """
    names = ["cde.claims.band", "cde.diagnose.engine",
             "cde.claims.band_prompts", "cde.refs.parser", "cde.refs.schema",
             ...
             "cde.claims.marker_scope",
             "cde.diagnose.strength", "cde.diagnose.provenance",
             "cde.diagnose.pipeline", "cde.refs.preband_contract"]
    if fulltext_path:
        names += ["cde.claims.coverage_prompts", "cde.claims.aggregate",
                  "cde.claims.fulltext", "cde.claims.spans"]
    if f5_seams is not None:
        names += ["cde.diagnose.activation", "cde.diagnose.supersession",
                  "cde.diagnose.candidate_screen",
                  "cde.diagnose.contradiction_prompt",
                  "cde.diagnose.f5_seams", "cde.diagnose.evidence_store",
                  "cde.refs.notice", "cde.diagnose.study_cluster",
                  "cde.diagnose.controversy_bundle",
                  "cde.diagnose.candidate_finder",
                  "cde.diagnose.discovery_queue"]
    ...
    if f7_seams is not None:
        names += ["cde.diagnose.entity", "cde.diagnose.evidence_builder",
                  "cde.diagnose.f7_seams"]
    out: dict = {}
    for name in names:
        try:
            mod = __import__(name, fromlist=["x"])
        except ImportError:
            # A module this run's wiring says governs a number, and which cannot
            # be imported, is not an optional field. Skipping it silently is how
            # the whole table came back empty after a package rename while every
            # manifest still validated.
            raise RuntimeError(
                f"{name} governs a number on this run's wiring but could not "
                f"be imported; refusing to write a manifest that omits its "
                f"digest") from None
        f = getattr(mod, "__file__", None)
        if f and os.path.exists(f):
            # KEYED BY PACKAGE-RELATIVE PATH, not basename. Since the package
            # was split, a basename no longer identifies a module, and the
            # manifest's job is to let a reader find the file a digest is about.
            out[name.split(".", 1)[1].replace(".", "/") + ".py"] = _sha256_file(f)
    return out
...
def _f5_manifest_block(f5_policy, f5_records, f5_runtime) -> dict:
    ...
    from .activation import ACTIVATION_SCHEMA_VERSION
    from .evidence_store import SOURCE_PACKET_SCHEMA_VERSION
    from ..refs.notice import NOTICE_RESOLVER_VERSION
    from .study_cluster import STUDY_CLUSTER_VERSION
    from .controversy_bundle import CONTROVERSY_BUNDLE_SCHEMA_VERSION
    from .candidate_screen import CANDIDATE_SCREEN_VERSION
    from .contradiction_prompt import RESPONSE_PARSER_VERSION
...
def run_natural_judgment(
    ...
    if f5_seams is not None:
        from .supersession import validate_f5_policy
    ...
    if f7_seams is not None:
        from .entity import validate_f7_policy
    ...
    if fulltext_path:
        from ..claims.coverage_prompts import COVERAGE_PROMPT_V3
    ...
    if f5_seams is not None and f5_runtime["judge_calls"] > 0:
        from .contradiction_prompt import F5_CONTRADICTION_PROMPT
        ...
        "f8": {
            ...
            "attestation": dict(
                ((disp_obj.check_attestations if disp_obj is not None else {})
                 .get("F8") or {})),
            "note": (
                "F8 is a pre-band decision produced by cde.refs.run using linked "
    ...
    if f5_seams is not None:
        from .discovery_queue import (QUEUE_FILENAME, assert_blind, build_queue)
        ...
        from .controversy_bundle import validate_controversy_bundle
