...
Usage:
    python -m cde.runtime.sandbox_judge packet.json --model claude-opus-5
    python -m cde.runtime.sandbox_judge packet.json --taxonomies F4,F6 --dry-run
"""
...
from ..diagnose import outcome as tox
from ..claims.band_prompts import (
...
def _abstract_fetcher(packet: dict):
    ...
    def fetch_abstract(pmid):
        ...
        if live:
            if key not in cache:
                from ..claims.abstracts import fetch_abstract as _live_abstract
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        prog="cde.runtime.sandbox_judge",
