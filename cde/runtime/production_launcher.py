...
from ..claims import citation_selection
...
GOVERNING_MODULES = (
    # PACKAGE-RELATIVE PATHS, not basenames. Since the package was split into
    # subpackages a basename no longer locates a file, and two subpackages may
    # legitimately hold the same one. These are joined to pkg_dir and are also
    # the keys of runtime_module_sha256 in the manifest, so a reader can find
    # the file a digest is about.
    #
    ...
    # the judgment modules.
    "runtime/production_launcher.py", "refs/run.py", "refs/lookup.py",
    "refs/confirm.py", "refs/decide.py", "refs/doi_lookup.py",
    "refs/biblio_match.py", "refs/resolve_a.py", "refs/work_identity.py",
    "refs/llm_filter.py", "refs/preband_disposition.py",
    "runtime/ratelimit.py", "refs/ncbi_meta.py", "refs/textnorm.py",
    "refs/unscoreable.py", "refs/journal_identity.py", "refs/samework.py",
    "refs/thresholds.py", "refs/eval_report.py", "refs/retraction.py",
    "diagnose/reasons.py", "diagnose/pipeline.py", "claims/band.py",
    "diagnose/engine.py", "claims/cocitation.py", "claims/band_prompts.py",
    "refs/parser.py", "refs/schema.py", "diagnose/provenance.py",
    "diagnose/strength.py", "diagnose/entity.py", "refs/preband_contract.py",
    "diagnose/evidence_builder.py", "diagnose/f7_seams.py",
    "runtime/parser_versions.py", "claims/coverage_prompts.py",
    "claims/aggregate.py", "diagnose/activation.py",
    "diagnose/candidate_screen.py", "diagnose/evidence_store.py",
    "refs/notice.py", "diagnose/study_cluster.py",
    "diagnose/controversy_bundle.py", "diagnose/supersession.py",
    "diagnose/f5_seams.py", "diagnose/candidate_finder.py",
    "diagnose/contradiction_prompt.py", "diagnose/discovery_queue.py"
)
    ...
    * **unsupported** -- DEC-070. The parameter is NOT SENT and is recorded as
    ...
    if temperature_unmeasured(model):
        ...
        raise LaunchRefused(
            ...
            f"`python -m cde.runtime.check_openai_judge` (check C) and move the id "
...
def _band1_attestations(log_path: str, *, snapshot_date: str) -> dict:
    ...
    return {
        "F1": {
            **common,
            "source": (
                "cde.refs.run exact identity pipeline: PubMed, DOI Foundation, "
        ...
        "F2": {
            **common,
            "source": (
                "cde.refs.run work-identity pipeline: PubMed, DOI Foundation, "
                "Crossref, DataCite, OpenAlex"),
...
def _ncbi_f3_seams(ncbi_key: str, email: str):
    ...
    from ..refs.ncbi_meta import ncbi_pmid_to_pmcid, ncbi_pmc_reflist
...
def _openalex_abstract_seam(mailto: str, api_key: str = ""):
    ...
    """
    import requests as _requests
    from ..refs.doi_lookup import fetch_openalex_abstract
    session = _requests.Session()
...
def launch_full(*, repo_dir: str, pkg_dir: str, xml_dir: str, out_dir: str,
    ...
    if not callable(run_kwargs.get("f3_resolve_pmcid")) and not callable(
        ...
        from ..refs.ncbi_meta import DEFAULT_EMAIL as _NCBI_DEFAULT_EMAIL
    ...
    from ..diagnose.f5_seams import validate_production_f5_configuration
    from ..diagnose.f7_seams import validate_production_f7_configuration
    try:
    ...
    from ..refs.run import make_completer, run as run_band1
    from ..refs.parser import iter_pmc_dir
