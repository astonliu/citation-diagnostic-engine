"""cde/runtime/check_openai_judge.py -- the four checks that need a live call.
...
RUN IT BEFORE TRUSTING A BATCH, and after any edit to ``usage_dict``:
    ...
    OPENAI_API_KEY=... python -m cde.runtime.check_openai_judge
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(prog="cde.runtime.check_openai_judge")
    ...
    print(f"All checks passed ({', '.join(sorted(results))}). Note that E "
          ...
          "not live checks: run "
          "`pytest -q cde/freeze/test_openai_judge_wiring.py "
          "cde/freeze/test_prompt_caching_and_usage.py`.")
    return 0
