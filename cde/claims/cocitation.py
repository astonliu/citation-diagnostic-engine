...
def group_record(group_id: str, items: "Sequence[dict]", aggregated: dict,
    ...
    return {
        ...
        **({"marker_scope_id": scope_id,
        ...
        "sentence_members": list(first.get("citance_group_members") or [
            i.get("citation_id") for i in items]),
        "sentence_size": len(first.get("citance_group_members") or items),
        "scope_members": [i.get("citation_id") for i in items],
