...
Cascade (stop at the first confident hit; cheapest/most reliable first):
    1. Cited DOI     -- when written_doi is present and differs from the resolved
                        DOI, dereference it (Crossref works/{doi}).
...
from .ratelimit import CROSSREF, NCBI, request_with_retry
...
def resolve_by_cited_doi(claimed: ClaimedRef, resolved_doi: str,
    ...
    """Step 1. When the citation carries its OWN DOI that differs from the
    resolved record's DOI, dereference it via Crossref works/{doi}. The cheapest,
    most reliable path -- a DOI is a globally unique work identifier. Returns None
    when there is no distinct cited DOI or the dereference finds nothing."""
