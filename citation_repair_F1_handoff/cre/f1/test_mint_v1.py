...
from cre.f1 import band_prompts
from cre.f1.freeze import bootstrap, fixtures_v1 as fx, mint_v1, schema_gate
from cre.f1.freeze.canon_v1 import canon_sha256, canon_v1
from cre.f1.freeze.semantic_validator_v1 import (FROZEN_SOURCE_BLOB_OID,
                                                 RULES, validate)
from cre.f1.freeze.strict_loader import load_strict

PYTHON = sys.executable
REPO_HANDOFF = pathlib.Path(__file__).resolve().parents[2]
REPO_ROOT = REPO_HANDOFF.parent
FREEZE = REPO_HANDOFF / "cre" / "f1" / "freeze"
...
def test_row18_mint_run_leaves_the_worktree_clean():
    ...
    status = subprocess.run(
        ["git", "-C", str(REPO_ROOT), "status", "--porcelain", "--",
         "citation_repair_F1_handoff/cre/f1/freeze"],
        capture_output=True, text=True)
    assert status.stdout.strip() == "", status.stdout
    ignored = subprocess.run(
        ["git", "-C", str(REPO_ROOT), "check-ignore", "-q",
         "citation_repair_F1_handoff/cre/f1/freeze/_mint_out/CONFIG_x.json"],
        capture_output=True, text=True)
